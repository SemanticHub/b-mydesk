...
<div class="content" style="background: #eee; padding: 0.5em">
    ...
    <div class="ui fluid form segment" style="background: rgba(0, 0, 0 , 0.1); margin-top: 0">
        <div style="display: block; border: 1px solid; margin-top: 0px;" class="ui info message">
            <ul>
                <li><label class="ui label">Package: </label> <?php echo Settings::model()->findByPk($model->item)->item; ?> <label class="ui label">Days: </label> <?php echo $model->days ?> ( <?php echo $model->fromdate ?> to <?php echo $model->todate ?> )</li>
                <li><label class="ui label">Location: </label> <?php echo $model->location; ?> </li>                
                <li>
                    <label class="ui label">Estimated Total: </label> BDT <?php echo $model->est_total; ?>
        ...
        <div class="field">
            ...
            <div class="ui mini input">
                ...
                echo $form->dropDownList(
                            $model, 
                            'team_id', 
                            CHtml::listData(Teams::model()->findAll(), 'id', 'name'), array(
                            'type' => 'text',
                            'class' => 'ui mini selection ',
                            'placeholder' => 'Select',
                            'empty' => '-Select-',
//                            'onchange' => CHtml::ajax(array(
//                            'type' => 'GET',
//                            'update' => '#Photography_team_member1s',
//                            'url' => array("teams/members"),
//                            array(
//                                'selected' => true
//                            )
//                            )
//                        )
        ...
        <div class="field">
            ...
            <?php echo $form->dropDownList($model, 'team_members', null, array('size' => '3', 'multiple' => 'true', 'type' => 'text', 'class' => 'ui mini selection ', 'placeholder' => 'Select')); ?>
...
<script type="text/javascript">
    $(function () {
        $('#Photography_team_id').on('change', function (e) {
            $.ajax({
                url: "/communications/teams/members?team_id=" + $('#Photography_team_id').val(),
                type: "GET",
                success: function (data) {
                    console.log(data);
                    $('#Photography_team_members').html(data);
                },
            });
            return false;
        });
        $('#save-assign').on('click', function (e) {
            ...
            $.ajax({
                url: "/communications/photography/assignToTeam?id=" + $('#Photography_id').val(),
                type: "POST",
