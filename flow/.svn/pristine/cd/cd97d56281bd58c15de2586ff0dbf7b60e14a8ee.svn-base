...
<div id="activity-controller-scope" ng-controller="ActivityController">
        ...
        <div class="column">
            <div class="ui stacked segment">
                <h2 class="ui header">
                    ...
                    <div class="content">
                        Marketplace <div class="ui orange tag label">&nbsp;{{queue}}</div>
                        <div class="sub header">প্রতীক্ষমাণ কুপন</div>
                    </div>
                </h2>
                ...
                <span ng-hide="queueTimesup">
                    <hr style="color: #ccc; border-top: 1px solid #eee; background: none;border: none;border-top: 1px dashed #ccc;" />
    ...
    <div class="ui warning icon message" style="margin-top: 0; margin-bottom: 10px">
        <div class="content">
            <div class="header">
                টাকা জমা করার জন্য আপনার বিকাশ মেনুর <i class="circular red mobile icon"></i> পেমেন্ট অপসন থেকে  ০১৭০ ৯৬ ৩৩ ৭৬০ নম্বর এ পেমেন্ট করুন। <br />
                যেমন: *247# <i class="right arrow  red icon"></i> 3. Payment <i class="right arrow  red icon"></i> 01709633760 <i class="right arrow  red icon"></i> Enter amount <i class="right arrow  red icon"></i> Reference: 1 <i class="right arrow  red icon"></i> Counter: 1 <i class="right arrow red icon"></i> Enter your PIN
            </div>    
        </div>
    </div>
    ...
    <div class="ui info icon message" style="margin-top: 0; margin-bottom: 10px">
        <div class="content">
            <div class="header">
                কুপন কিনতে নিচের ক্যালেন্ডার  <i class="circular inverted red calendar icon"></i> এর নির্দিষ্ট তারিখ এ ক্লিক করুন। 
                বাতিল করার জন্য উক্ত টোকেন এর <i class="circular inverted red remove icon"></i> আইকন এ ক্লিক করুন।
                ট্রান্সফার করার জন্য উক্ত টোকেন এর<i class="right arrow circular inverted red icon"></i> আইকন এ ক্লিক করুন।
            </div>    
        </div>
    </div>
</div>
...
<script type="text/javascript">
                                ...
                                var showAlertMessage = function (message) {
                                    $('#modal-message').html(message);$('.ui.basic.modal').modal({closable: false}).modal('show');
                                }
                                $.ajax({
                                    type: "GET", url: "/inn/holiday/index", dataType: 'json', async: false,
                                    success: function (data) {if (data) holidays = data;},
                                });
                                ...
                                myDesk.controller('ActivityController', ['$scope', function ($scope) {
                                        ...
                                        $scope.userQueue = 0; // user request from marketplace
                                        $scope.todayToken = 0;
                                        $scope.todayQueue = 0;
                                        $scope.unitPrice = 45;
                                        ...
                                        $scope.userQueueCount = function () {
                                            $.ajax({
                                                type: "GET", url: "userQueueCount", dataType: 'json',
                                                success: function (data) {
                                                    if (data) {
                                                        if (data.status == "success") {
                                                            $scope.$apply(function () {$scope.userQueue = data.count;});
                                                        } else {
                                        ...
                                        $scope.userQueueAdd = function () {
                                            if ($scope.balance >= (parseInt($scope.userQueue) + 1) * $scope.unitPrice) {
                                                $.ajax({
                                                    type: "GET", url: "userQueueAdd", dataType: 'json',
                                                    success: function (data) {
                                                        if (data) {
                                                            if (data.status == "success") {
                                                                $scope.$apply(function () {$scope.userQueue = parseInt($scope.userQueue) + 1;});
                                                            } else {
                                                                alert("Failed to communicate with the server.");
                                                            }
                                                        }
                                        ...
                                        $scope.userQueueRemove = function () {
                                            if ($scope.userQueue > 0) {
                                                $.ajax({
                                                    type: "GET", url: "userQueueRemove", dataType: 'json',
                                                    success: function (data) {
                                                        if (data) {
                                                            if (data.status == "success") {
                                                                $scope.$apply(function () {$scope.userQueue = parseInt($scope.userQueue) - 1;});
                                        ...
                                        $scope.reloadBalance = function () {
                                            $.ajax({
                                                type: "GET", url: "balance", dataType: 'json', data: {pin: $('#user-pin').val()},
                                                success: function (data) {
                                                    if (data) {
                                                        if (data.status == "success") {
                                                            //var scope = angular.element($("#activity-controller-scope")).scope();
                                                            $scope.$apply(function () {$scope.balance = data.balance;});
                                                        } else {
                                                            alert("Failed to communicate with the server.");
                                                        }
                                                    }
                                        ...
                                        $scope.reloadMarket = function () {
                                            $.ajax({
                                                type: "GET", url: "queue", dataType: 'json',
                                                success: function (data) {
                                                    var scope = angular.element($("#activity-controller-scope")).scope();
                                                    if (data) {
                                                        if (data.status == "available") {
                                                            scope.$apply(function () {
                                                                scope.queue = data.queue.length;
                                                                scope.market = data.queue;
                                                                scope.queueEmpty = false;
                                                            });
                                        ...
                                        $scope.reloadTokens = function () {
                                            $.ajax({
                                                type: "GET", url: "alltokens", dataType: 'json', data: {pin: $('#user-pin').val()},
                                                success: function (data) {
                                                    if (data) {
                                                        var events = new Array();
                                                        $.each(data, function (i, v) {
                                                            var event = {title: "T #" + v.id, start: v.date_time, id: v.id};
                                                ...
                                                success: function (data) {
                                                    if (data) {
                                                        if (data.status == "success") {
                                                            ...
                                                            var event = {title: "T #" + data.id, start: data.date_time, id: data.id};
                                                            $('#calendar').fullCalendar('renderEvent', event, true);
                                                    ...
                                                    $.ajax({
                                                        ...
                                                        success: function (data) {
                                                            if (data && data.status == "success") {
                                                                ...
                                                                var event = {title: 'T #' + data.serial, start: date, id: data.serial};
                                                                $('#calendar').fullCalendar('renderEvent', event, true);
                                                            } else {
