...
class CasesController extends Controller {
    ...
    public $violanceTypes = array(
            '1' => 'Acid attack',
            '2' => 'Rape',
            '3' => 'Gang rape',
            '4' => 'Attempted rape',
            '5' => 'Attempted gang rape',
            '6' => 'Rape and Murder',
            '7' => 'Rape and attempted murder',
            '8' => 'Murder',
            '9' => 'Attempted murder',
            '10' => 'Lured Suicide',
    ...
    public function accessRules() {
        return array(
            array('allow', // allow authenticated user to perform 'create' and 'update' actions
                ...
                //'users' => array('@'),
                'expression' => 'Supervisors::model()->isAgent()',
            ),
            array('deny',
                'message' => 'You are not authorized to access this service. To report any violence incident against women and children, please call 09 666 799 799 for a VAWC support agent. Contact ICT support if you want to use this service.',
                'users' => array('*'),
            ),
        );
    }
    ...
    public function actionCreate() {
        ...
        if (isset($_POST['Cases'])) {
            ...
            $groups_for_sms = $_POST['groups'];
            //Yii::app()->end();
            if ($model->save()) {
                $successCount = 0;
                $errorCount = 0;

                $gm = Yii::app()->db->createCommand()
                        ->from('vawc_members m, vawc_group_members gm')
                        //->where('m.id=gm.member_id and gm.group_id=:gid', array(':gid' =>  $groups_for_sms[0]))
                        ->where('m.id=gm.member_id')
                        ->andWhere(array('in', 'gm.group_id', $groups_for_sms))
                        ->queryAll();
                //CVarDumper::dump($gm, 10, true);
                //Yii::app()-end();
                //$groupMembers = SmsGroupMembers::model()->findAll('group_id=:group_id', array(':group_id' => $model->groupId));
                foreach ($gm as $k => $v) {
                    $out = new SmsOut();
                    $out->group_id = $v['group_id'];
                    $out->message = 'CASEID ' . $model->id . ', ' . $violanceTypes[$model->violance_type] . ', ' . $model->case_summary . ', ' . $victimCondition[$model->victim_current_condition];
                    $out->message_type = '1'; // 1 for text, 2 for flash, 3 for unicode
                    $out->department = 'VAWC';
                    $out->mask = 'BRAC';
                    $out->to_number = '0' . $v['phone'];
        ...
        $this->render('create', array(
            'model' => $model,
            'violanceTypes' => $this->violanceTypes,
